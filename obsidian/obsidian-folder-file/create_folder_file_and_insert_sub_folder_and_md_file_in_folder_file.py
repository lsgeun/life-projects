...
not_included_directory = ["0 Attachment"]
...
while not (directory_paths == []):
    ...
    # ^ 이 부분을 주석 처리하면 하위 디렉토리, MD 파일을 찾는 새로운 파일을 만들지 않고 기존의 디렉토리 파일의 davaview 코드 블록이 삭제됨
    ...
    "# Directory\n"+\
    "## Parent\n"+\
    "```dataview\nTABLE file.mday as 수정일, file.cday as 생성일, file.size as \"파일 크기\"\nWHERE\n\t(\n\t\t(length(split(this.file.folder, \"/\")) = 1 AND this.file.folder = \"\"\n\t\tAND false)\n\tOR\n\t\t(length(split(this.file.folder, \"/\")) = 1 AND this.file.folder != \"\"\n\t\tAND file.name = \"0 NOTE-iCloud_Drive\")\n\tOR\n\t\t(length(split(this.file.folder, \"/\")) >= 2 AND length(split(regexreplace(replace(this.file.folder, file.folder, \"\"), \"^/\", \"\"),  \"/\")) = 1 AND length(split(file.folder, \"/\")) != length(split(this.file.folder, \"/\"))\n\t\tAND (file.name = \"0\" + \" \" + regexreplace(split(file.folder,  \"/\")[length(split(file.folder,  \"/\"))-1], \"^\d+ \", \"\")\n\t\tOR file.name = \"0\" + \" \" + split(file.folder,  \"/\")[length(split(file.folder,  \"/\"))-1]\n\t\tOR file.name = \"0\" + \" - \" + split(file.folder,  \"/\")[length(split(file.folder,  \"/\"))-1]))\n\t)\n\tAND file.name != this.file.name\n\t\nSORT file.name ASC\n```"+\
    "\n\n"+\
    "## Child\n"+\
    "```dataview\nTABLE file.mday as 수정일, file.cday as 생성일, file.size as \"파일 크기\"\nWHERE\n\t(\n\t\t(length(split(this.file.folder, \"/\")) = 1 AND this.file.folder = \"\"\n\t\tAND length(split(file.folder, \"/\")) = 1 AND file.folder != \"\")\n\tOR\n\t\t(length(split(this.file.folder, \"/\")) = 1 AND this.file.folder != \"\"\n\t\tAND length(split(regexreplace(replace(file.folder, this.file.folder, \"\"), \"^/\", \"\"),  \"/\")) = 1 AND length(split(file.folder, \"/\")) != 1)\n\tOR\n\t\t(length(split(this.file.folder, \"/\")) >= 2\n\t\tAND length(split(regexreplace(replace(file.folder, this.file.folder, \"\"), \"^/\", \"\"),  \"/\")) = 1 AND length(split(file.folder, \"/\")) != 1 AND length(split(file.folder, \"/\")) != length(split(this.file.folder, \"/\")))\n\t)\n\tAND (file.name = \"0\" + \" \" + regexreplace(split(file.folder,  \"/\")[length(split(file.folder,  \"/\"))-1], \"^\d+ \", \"\")\n\t\tOR file.name = \"0\" + \" \" + split(file.folder,  \"/\")[length(split(file.folder,  \"/\"))-1]\n\t\tOR file.name = \"0\" + \" - \" + split(file.folder,  \"/\")[length(split(file.folder,  \"/\"))-1])\n\tAND file.name != this.file.name\nSORT file.name ASC\n```"+\
    "\n"
    ...
    # not_included_directory에 속한 디렉토리를 제외한 현재 디렉토리에 있는 숨겨지지 않은 하위 디렉토리
    current_sub_directory_names_without_hidden = [_ for _ in current_sub_directory_names if _.find(".") == -1]; current_sub_directory_names_without_hidden = [_ for _ in current_sub_directory_names_without_hidden if _ not in not_included_directory]
